...
(
	IF "%src:~-1%"=="\" ( REM source is a directory
		MKLINK /D "%dst%%srcnx%" "%src:~0,-1%" || MKLINK /J "%dst%%srcnx%" "%src:~0,-1%" || "%COMMANDER_PATH%\bin\xln.exe" -n "%src:~0,-1%" "%dst%%srcnx%"
	) ELSE (
		rem fsutil requires admin rights --
		MKLINK /H "%dst%%srcnx%" "%src:~0,-1%" || MKLINK "%dst%%srcnx%" "%src:~0,-1%" || "%COMMANDER_PATH%\bin\xln.exe" "%src:~0,-1%" "%dst%%srcnx%" || fsutil hardlink create "%dst%%~nx1" %1
	)
