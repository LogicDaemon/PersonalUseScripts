...
SET distURL=http://releases.mozilla.org/pub/mozilla.org/firefox/releases/latest/win32/ru/
SET distfname=Firefox Setup 

IF NOT EXIST "%workdir%" MKDIR "%workdir%"
START "" /B /WAIT /D"%workdir%" "c:\SysUtils\wget.exe" -ml2 -H -nd -np -e robots=off -A.exe,.asc -o"%~n0.log" %distURL%

DEL "%workdir%\Firefox Setup Stub *.exe"
...
:linkdst
    SET SU_Aversion=%~n1
    REM len(%distfname%)=14
    SET SU_Aversion=%SU_Aversion:~14%
    REM linking
    ...
    IF NOT EXIST "%dst%" (
        ...
        IF DEFINED SUScripts MOVE "%SUScripts%\Mozilla FireFox *.cmd" "%SUScripts%\Mozilla FireFox %SU_Aversion%.cmd"
    )
EXIT /B
