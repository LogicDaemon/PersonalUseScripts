...
Remove=%RemoveOtherComponents%`,%RemoveLanguages%
;,%RemoveDictionaries%
...
InstallMSI(MSIFileFullPath, params){
    ...
    RunWait msiexec.exe /i "%MSIFileFullPath%" %params% /norestart /liwearucmop "%LogPath%",, UseErrorLevel
    
    return CheckError(ErrorLevel, "after attempt to run`n""" . MSIFileFullPath . """ %params%`n`nLog written to """ . LogPath . """.")
}

CheckError(ReturnErrValue, Description) {
    FileAppend Error installing: %ReturnErrValue% %Description%, *
    If (ReturnErrValue && RunInteractiveInstalls)
