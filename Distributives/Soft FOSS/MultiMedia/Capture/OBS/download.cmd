...
@(
    ...
    IF "%~x1"==".torrent" IF EXIST "%~dp0%~nx1" (
        START "" /B aria2c --file-allocation=trunc --enable-dht6 --seed-time=0 --bt-detach-seed-only --bt-hash-check-seed=false --auto-file-renaming=false --check-integrity=true -T "%~dp0%~nx1"
    )
    EXIT /B
)
