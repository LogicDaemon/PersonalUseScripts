...
def download_obs() -> None:
    """ Parse the OBS download page, download the files """

    base_url = httpx.URL('https://obsproject.com/')
    with httpx.Client(http2=config.http2) as client:
        r = client.get(base_url)
        r.raise_for_status()
        page = lxml.html.fromstring(r.text)
        # //*[@id="home_downloads"]/a[1]
        dl_url = page.xpath('//*[@id="home_downloads"]/a[1]/@href')
        log.debug('Download URL: %s', dl_url)
        with Progress() as progress:
            download(client, base_url.join(dl_url[0]), progress=progress)
