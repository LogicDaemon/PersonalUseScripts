...
IF "%~dp0"=="" (SET "srcpath=%CD%\") ELSE SET "srcpath=%~dp0"
    ...
    :: 1 = found & unpacked to PlugIns\wcx\Total7zip complete distributive
    :: 2 = copied exe7z ("%utilsdir%%exename7za%" if not predefined)
    SET "total7zip7zgexeStatus="
...
IF NOT DEFINED total7zip7zgexeStatus (
    REM fallback in case 7-zip distributive was not found
    IF NOT EXIST "%TCDir%\PlugIns\wcx\Total7zip" MKDIR "%TCDir%\PlugIns\wcx\Total7zip"
    ECHO N|COPY /B %exe7z% "%TCDir%\PlugIns\wcx\Total7zip\7zg.exe"
    SET exe7z="%TCDir%\PlugIns\wcx\Total7zip\7zg.exe"
    SET "total7zip7zgexeStatus=2"
)
(
	...
	IF NOT EXIST "%TCDir%\PlugIns\wcx\Total7zip\64\7zg.exe" IF EXIST "%TCDir%\PlugIns\wcx\Total7zip\7zg.exe" "%TCDir%\xln.exe" "%TCDir%\PlugIns\wcx\Total7zip\7zg.exe" "%TCDir%\PlugIns\wcx\Total7zip\64\7zg.exe" || COPY /B "%TCDir%\PlugIns\wcx\Total7zip\7zg.exe" "%TCDir%\PlugIns\wcx\Total7zip\64\7zg.exe"
    ) ELSE (
    ...
    IF NOT EXIST "%TCDir%\7zg.exe" (
        IF EXIST "%TCDir%\PlugIns\wcx\Total7zip\64\7zg.exe" (
            MKLINK "%TCDir%\7zg.exe" "%TCDir%\PlugIns\wcx\Total7zip\64\7zg.exe" || "%TCDir%\xln.exe" "%TCDir%\PlugIns\wcx\Total7zip\64\7zg.exe" "%TCDir%\7zg.exe" || COPY /B "%TCDir%\PlugIns\wcx\Total7zip\64\7zg.exe" "%TCDir%\7zg.exe"
        ) ELSE (
            MKLINK "%TCDir%\7zg.exe" "%TCDir%\PlugIns\wcx\Total7zip\7zg.exe" || "%TCDir%\xln.exe" "%TCDir%\PlugIns\wcx\Total7zip\7zg.exe" "%TCDir%\7zg.exe" || COPY /B "%TCDir%\PlugIns\wcx\Total7zip\7zg.exe" "%TCDir%\7zg.exe"
        )
    )
    CALL :GetFileNameExt AutohotkeyExeName %AutohotkeyExe%
)
(
    rem hardlinking from Program Files causes weird permissions after profile permissions reset --- "%utilsdir%xln.exe" %AutohotkeyExe% "%TCDir%\AutoHotkey.exe" || 
    MKLINK "%TCDir%\AutoHotkey.exe" %AutohotkeyExe% || XCOPY %AutohotkeyExe% "%TCDir%\*.*" /I /Y || (
	@ECHO Error copying AutoHotkey to TC dir "%TCDir%"
	PING 127.0.0.1 -n 3 >NUL
    )
    "%TCDir%\xln.exe" "%TCDir%\%AutohotkeyExeName%" "%TCDir%\Autohotkey.exe" || MKLINK "%TCDir%\Autohotkey.exe" "%TCDir%\%AutohotkeyExeName%" || COPY /B /Y "%TCDir%\%AutohotkeyExeName%" "%TCDir%\Autohotkey.exe"
...
(
    ...
    IF DEFINED total7zip7zgexeStatus IF %newlocal7zType% GEQ %total7zip7zgexeStatus%. (
        ECHO Already unpacked %total7zip7zgexeStatus%-bit 7-Zip locally, ignoring %newlocal7zType%-bit one from %1
        EXIT /B
    )
    CALL :FindFirstExisting exe7z "%TCDir%\PlugIns\wcx\Total7zip%outSubdir%\7z.exe" "%TCDir%\PlugIns\wcx\Total7zip%outSubdir%\7zg.exe" && SET "total7zip7zgexeStatus=1"
    EXIT /B
)
