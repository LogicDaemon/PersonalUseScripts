...
IF NOT DEFINED APPDATA IF EXIST "%USERPROFILE%\Application Data" SET "APPDATA=%USERPROFILE%\Application Data"
    ...
    IF NOT EXIST "%ProgramData%\Common_Scripts" (
	IF EXIST "%SystemDrive%\Common_Scripts" (
	    MOVE /Y "%SystemDrive%\Common_Scripts" "%ProgramData%\Common_Scripts"
	) ELSE MKDIR "%ProgramData%\Common_Scripts"
    )
...
(
    %exe7z% x -r -aoa "%~dpn0.7z" -o"%ProgramData%\Common_Scripts" || %ErrorCmd%
    %windir%\System32\compact.exe /C /EXE:LZX /S:"%ProgramData%\Common_Scripts" /I /Q || %windir%\System32\compact.exe /C /S:"%ProgramData%\Common_Scripts" /I /Q
    
    SET "PATH=%PATH%;%ProgramData%\Common_Scripts"
    %AutohotkeyExe% "%utilsdir%pathman.ahk" /as "%ProgramData%\Common_Scripts"
