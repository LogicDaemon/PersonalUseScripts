Affinity_Set( ByRef PAM=0xFF, ByRef PID=0x0, ByRef old_PAM := 0, ByRef SAM := 0) { ; CPU0=1 CPU1=2 | to use both, PAM should be 3
    local ; Force-local mode
    If (PID) {
        Process Exist, %PID%
        If (!ErrorLevel)
            Throw Exception("Process with specified PID does not exist",, PID)
    } Else {
        PID := DllCall( "GetCurrentProcessId" )
    }
    hPr := DllCall( "OpenProcess",Int,1536,Int,0,Int,PID )  
    DllCall( "GetProcessAffinityMask", "Int", hPr, "Int*", old_PAM, "Int*", SAM )
    Res := DllCall( "SetProcessAffinityMask", "Int", hPr, "Int", PAM := PAM & SAM ), lastError := A_LastError
    ...
    Return ( Res="" ) ? 0 : Res, A_LastError := lastError
}
