...
rem -x --exclude
rem -X --excludedir
ln -X .mypy_cache -X .venv -X connect-asg-host -X helper_shortcuts -X Photo -X temp-backup-scripts -X Soft_old -x Hotkeys_Custom.ahk -x Hotkeys_Custom.*.ahk -x *.lnk -x _Distributives.base_dirs.txt -x aws_s.cmd -x aws_t.cmd --unroll --recursive --mirror  "%LOCALAPPDATA%\Scripts" "%~dp0misc"
...
unison Distributives_scripts_in_git
)
