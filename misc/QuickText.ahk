...
Global linesFullContents := [] ; Full contents of each line, including separators
	; Displayed part is almost the same but cut to 100 chars
	, displayLines := []
	; Contents of each line split into fields, stored in a 2D array
	, DB := []
	; List of line numbers that match the search criteria
	, hitlist := []
	, Choice
...
; linesFullContents: Complete content of each line, including delimiters. Generated by ReadDBFile(DatabaseFile).
; Choice: Position of the selected item in the listbox or hitlist. Generated by the listbox in the GUI (vChoice) or set to 1 by the input subroutine.
...
GUIStart:
	...
	Gui Add, Edit, gWindowInput vEingabe ys w115 h20 +Left,
	Gui Add, ListBox, Section xm w160 h260 gShiftSelection vChoice altsubmit
...
WindowInput(CtrlHwnd, GuiEvent, EventInfo, ErrLevel:="") {
	Local
	Global hitlist, StartList, Direction, ModeNr, linesFullContents, Choice, Eingabe
	Static SearchFor := ""
...
ShowSelectedText() {
	...
	Local
	Global hitlist, Choice, linesFullContents
	toShow := StrReplace(linesFullContents[hitlist[Choice]], "¤", "`n")
...
Join(sep, ByRef linesFullContents) {
	Local
	lst := ""
	For _, line in linesFullContents {
		lst .= line sep
	}
	return SubStr(lst, 1, -StrLen(sep))
}

FilterJoin(sep, ByRef keys, ByRef linesFullContents) {
	Local
	lst := ""
	For _, key in keys {
		lst .= linesFullContents[key] sep
	}
	return SubStr(lst, 1, -StrLen(sep))
}
...
SearchDB(SearchFor) {
	...
	Local
	Global DB, CaseSensitive, WholeWordCheck, WordStart, WhichColumn, linesFullContents  ;Bestehende globale Variablen müssen bei Verwendung innerhalb einer Funktion als global deklariert werden
	matchingLineNums := []
	For lineNo, SearchIn in linesFullContents {
		If (WhichColumn)
...
ReadDBFile(DatabaseFile) {
	Local
	ReadDBFileAppendLine(GetHostname())
	ReadDBFileAppendLine(A_UserName "@intermedia.com")
	Loop Read, %DatabaseFile%
		ReadDBFileAppendLine(A_LoopReadLine)
}

ReadDBFileAppendLine(text) {
	Global hitlist, linesFullContents, displayLines, DB, Separator
	Static lastIndex := 0
	lastIndex++
	linesFullContents.Push(text)
	displayLines.Push(SubStr(text, 1, 100))
	DB[lastIndex] := StrSplit(text, Separator)
	hitlist[lastIndex] := lastIndex		
}
