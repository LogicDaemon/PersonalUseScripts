...
REM This work is licensed under a Creative Commons Attribution-ShareAlike 4.0 International License <http://creativecommons.org/licenses/by-sa/4.0/deed.ru>.
	...
	@REM ECHO 	findpath="%findpath%"
	@REM @REM 		relative path for unix find, which performed when seeking %distfmask% for files to link
	@REM ECHO 	findargs="%findargs%"
	@REM @REM		arguments for find. Default: -name %distfmask%
	...
	IF NOT DEFINED xlnexe CALL :FromPathOrFirstExisting xlnexe "%LocalAppData%\Programs\bin\xln.exe" "%LocalAppData%\Programs\SysUtils\xln.exe" "%~d0Distributives\Soft\PreInstalled\utils\xln.exe" || ECHO xln.exe not found
	@REM IF NOT DEFINED findexe CALL :FirstExisting findexe "%LocalAppData%\Programs\scoop\apps\git\current\usr\bin\find.exe" "%LocalAppData%\Programs\SysUtils\UnxUtils\find.exe" "%LocalAppData%\Programs\UnxUtils\find.exe" "%SystemDrive%\SysUtils\UnxUtils\find.exe" || ECHO posix find.exe not found
	IF NOT DEFINED wgetexe CALL :FromPathOrFirstExisting wgetexe "%LocalAppData%\Programs\scoop\apps\wget\current\wget.exe" "%LocalAppData%\Programs\bin\wget.exe" || ECHO wget.exe not found
	...
	IF "%~1"=="" (
		...
		ECHO Usage:
		ECHO %0 URL [distributiveFileMask ["wget parameters"] ]
		EXIT /B 32767
	)
)
...
(
	...
	IF DEFINED findargs ECHO ***WARNING**** posix find is not supported anymore
	@REM IF DEFINED findpath ( CALL :CheckFindPath ) ELSE SET "findpath=."
	@REM IF NOT DEFINED findargs (
	@REM 	@REM this suffix appears with "@" in place of "?" in URL when downloading with some wget versions
	@REM 	SET findargs=-name "%distfmask%" -o -name "%distfmask%@*"
	@REM )
)
(
FOR /F "usebackq delims=" %%I IN (`DIR /O-D /S /B "%workdir%%distfmask%"`) DO @(
	SET "dstfname=%%~I"
	GOTO :foundDistByMask
)
EXIT /B 1
)
:foundDistByMask
