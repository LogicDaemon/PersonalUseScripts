...
If (vscodeRunning && !scriptcmdln) {
    ...
    vscodeexe := FirstExisting(LocalAppData "\Programs\Microsoft VS Code\Code.exe", LocalAppData "\Programs\VS Code\Code.exe", ProgramFiles "\Microsoft VS Code\Code.exe")
    If (!vscodeRunning) {
        curlerr := -1
