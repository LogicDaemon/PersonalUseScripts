...
DownloadVSCode(ByRef distDir, ByRef vsCodeDest, insiders := False) {
    ...
    If (updInfoRaw) {
        ...
        If (insiders) {
            ; VSCode-win32-x64-1.104.0-insider-fbefb06cae2ebf8209b5c3d77813bcb4325f1764.zip
            path := distDir "\" dlNameNoExt "-" updInfo.version "." dlExt
                , distPathMask := distDir "\" dlNameNoExt "-" updInfo.version ".*"
        } Else {
            ; VSCode-win32-x64-1.103.2.zip
            path := distDir "\" dlName
    ...
    If (newVerURL) { ; newVerURL is only available when updInfoRaw is known; in these cases, path will also be defined
        ...
        cmdline = %SystemRoot%\System32\curl.exe -RL -z "%path%" -o "%pathtmp%" -- "%newVerURL%"
    } Else {
        ...
        cmdline = %SystemRoot%\System32\curl.exe -RLO -- "%newVerURL%"
    }
    ...
    If (ErrorLevel) {
        ...
        If (path) {
            ...
            expectedDistNamePrefix := "VSCode-" urlSuffixArch "-"
            If (pathtmp) {
                ...
                Loop Files, %distDir%\%expectedDistNamePrefix%*.*
                {
                    ...
                    If (A_LoopFileTimeModified > latestTime) {
                        If (insiders) ; Store files to delete later, to avoid messing up the Loop
                            deleteFiles.Push(path)
                        latestTime := A_LoopFileTimeModified
                            , path := A_LoopFileFullPath
                    }
                }
                If (insiders) ; Delete all but the latest
                    For _, f in deleteFiles
                        FileDelete %f%
            }
        }
        If (path) {
