...
from typing import (DefaultDict, MutableMapping, MutableSequence, MutableSet,
                    NoReturn, Optional, Set)
...
def main() -> NoReturn:
    ...
    if log.isEnabledFor(logging.INFO):
        ...
        for ino, dirsfilenames in inode_dir_filenames.items():
            ...
            for dirname, filenames_set in dirsfilenames.items():
                if len(filenames_set) == 1:
                    log.info('\t%s\\%s', dirname, iter(filenames_set).__next__())
                else:
                    log.info('\t%s:', dirname)
...
def find_hardlinks(
    ...
    for de in os.scandir(dir):
        ...
        if not de.is_file(follow_symlinks=False):
            continue
        if mask is not None and not fnmatch.fnmatch(de.name, mask):
            continue
